...
from scintillator_display.display.impl_a.scintillator_structure import Plane
...
class App(MathDisplayValues):
    def __init__(self, init_mode=('data', 'debug', 'demo')):
        ...
        self.data_manager = Data(impl_constant=self.true_scaler, impl="a",
                                 ...
                                 store_normals=True,
                                 mode=init_mode)
        self.plane = Plane(data_manager=self.data_manager, scale=scale, true_scaler=self.true_scaler)
        self.cube = cube.Cube()
